#!/usr/bin/env python3
"""Canonical-lane closure guard for the unlikely intersections workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['UIE_G1', 'UIE_G2', 'UIE_G3', 'UIE_G4', 'UIE_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['UIE_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['UIE_GM'] = 'PASS' if gm_pass else 'FAIL'
    blockers = {}
    ...
    blockers['UIE_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['UIE_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    normalized_gates = {'G1': gates['UIE_G1'], 'G2': gates['UIE_G2'], 'G3': gates['UIE_G3'], 'G4': gates['UIE_G4'], 'G5': gates['UIE_G5'], 'G6': 'NA', 'GCoh': gates['UIE_G6'], 'GM': gates['UIE_GM']}
    normalized_blockers = {'G1': blockers['UIE_G1'], 'G2': blockers['UIE_G2'], 'G3': blockers['UIE_G3'], 'G4': blockers['UIE_G4'], 'G5': blockers['UIE_G5'], 'G6': [], 'GCoh': blockers['UIE_G6'], 'GM': blockers['UIE_GM']}
