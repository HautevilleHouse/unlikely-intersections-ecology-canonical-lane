...
In-paper anchor: `paper/UNLIKELY_INTERSECTIONS_ECOLOGY_PREPRINT.md` (`UIE_G2`).

## Goal
Expand the compressed capture/restart language into the local-to-global transport gate for `proving closure of atypical and anomalous intersection families across admissible ambient ecologies through a multi-lane unlikely-intersection ecology super-architecture`.
...
`UIE_G2` closes when `sigma_counting` survives admissible losses and restart corrections: defect stays above capture floor across admissible losses.
This is the transport contribution to `M_UIE`.
...
If transport accounting and restart preservation hold, then `UIE_G2` carries the local control forward without breaking admissibility.
