...
## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving closure of atypical and anomalous intersection families across admissible ambient ecologies through a multi-lane unlikely-intersection ecology super-architecture`.
...
2. the endpoint-transfer lock `intersection_lock` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_UIE` remains positive.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_UIE` is meaningful.
